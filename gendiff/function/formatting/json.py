...
def style_json(diff_dict):
    ...
    for k, v in sorted_dict.items():
        if k in gendiff_func.get_no_change(diff_dict).keys():
            file_json.update({k: v})
        if k in gendiff_func.get_add(diff_dict).keys():
            temp_dict = {f'+{k}': v}
            file_json.update(temp_dict)
        if k in gendiff_func.get_sub(diff_dict).keys():
            temp_dict = {f'-{k}': v}
            file_json.update(temp_dict)
        if k in gendiff_func.get_inner_change(diff_dict).keys():
